    ...
    <group ref={group}>
      <Float speed={1.5} rotationIntensity={0.3} floatIntensity={0.8}>
        ...
        <RoundedBox args={[2.8, 0.7, 0.09]} radius={0.08} position={[0, 1.5, 0.02]}>
          <meshStandardMaterial color="#f16a24" emissive="#f16a24" emissiveIntensity={0.15} />
        </RoundedBox>

        {/* Data rows */}
        {[-0.1, -0.5, -0.9, -1.3].map((y, i) => (
          <RoundedBox key={i} args={[2.4 - i * 0.15, 0.08, 0.01]} radius={0.02} position={[-0.1, y, 0.05]}>
            <meshStandardMaterial color={i === 0 ? '#f16a24' : '#515f74'} opacity={0.4 - i * 0.06} transparent />
          </RoundedBox>
        ))}
        ...
        <RoundedBox args={[1.8, 0.12, 0.015]} radius={0.03} position={[-0.3, -1.7, 0.06]}>
          <meshStandardMaterial color="#f16a24" emissive="#f16a24" emissiveIntensity={0.1} />
        </RoundedBox>
      </Float>
    </group>
  );
...
function Particles({ count = 30 }) {
  ...
  return (
    <points ref={ref}>
      ...
      <pointsMaterial size={0.04} color="#f16a24" transparent opacity={0.3} sizeAttenuation />
    </points>
  );
}
